...
export class GeminiService {
  ...
  async generateContent(prompt: string, retries = 3): Promise<MedicineData> {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        return this.validateMedicineData(parsedData);
      } catch (error) {
        console.error(`Error in generateContent (attempt ${attempt}):`, error);

        if (attempt < retries) {
          console.warn(`Retrying in 2 seconds...`);
          await this.wait(2000);
        } else {
          throw new GeminiError("Failed to generate medicine information after multiple attempts", {
  ...
  private validateMedicineData(parsedData: Partial<MedicineData>): MedicineData {
